...
(function () {
  ...
  // ===== SOFORT exportieren =====
  window.__SPRECHB_VERSION__ = '1.3.0';
...
// ############### Ab hier Originalcode einfügen ###############
  ...
  function updateProgress(extra = '') {
    const progressText =
  //    `🚑 ${patientsDone}/${totalRequests} | 🚓 ${prisonersDone}/${totalRequests} | ⏭️ ${skipped}` +
      (extra ? ` | ${extra}` : '');
      `🚑 ${patientsDone}/${totalRequests} | 🚓 ${prisonersDone}/${totalRequests}`+ (extra ? ` | ${extra}` : '');
  ...
  async function handlePrisonTransport(baseUrl) {
    ...
    if (!btn || !btn.href) {
      ...
      ticker('⏭️ Kein Gefangenentransport verfügbar');
      return false; // wichtig: NICHT als Skip zählen
    }
  ...
  async function processAll() {
    ...
    ticker('▶️ Abarbeitung gestartet');
    const failures = [];
    ...
    try {
      ...
      for (let i = 0; i < vehicleLinks.length; i++) {
        ...
        for (let attempt = 0; attempt <= PRISON_RETRIES; attempt++) {
          try {
            ...
            if (done === false) {
              // kein Transport vorhanden → kein Skip, einfach weiter
              break;
            }
            ...
            if (attempt === PRISON_RETRIES) {
              ...
              console.warn('⏭️ Übersprungen (Fehler):', href, e);
              updateProgress('⚠️ Skip');
              ticker('⏭️ Fehler beim Gefangenentransport');
            } else {
      ...
      ticker(`✅ Fertig: ${patientsDone} 🚑 | ${prisonersDone} 🚓 | ${skipped} ⏭️`);
      console.log('[SprechwunschBot] fertig', { patientsDone, prisonersDone, skipped, totalRequests });

      // Kein Reload (wie von dir gewünscht)
  ...
  function addButton() {
    ...
    // Direkt nach der Button-Zeile einfügen
